# =============================================================================
# PUBLICATION-GRADE TGARCH-X ROLLING VOLATILITY FORECASTING
# =============================================================================
# Purpose
...
CODE_VERSION <- "tgarch_x_reviewer_aligned_v4_04_2016_2026"

# Run PRIMARY first to create the files imported by Python. Then rerun with
# WINDOW, DISTRIBUTION, and X for the reviewer-requested sensitivity analyses.
RUN_PROFILE <- toupper(Sys.getenv("NSE_RUN_PROFILE", unset = "PRIMARY"))
ALLOWED_RUN_PROFILES <- c("PRIMARY", "WINDOW", "DISTRIBUTION", "X", "ALL")
if (!RUN_PROFILE %in% ALLOWED_RUN_PROFILES) {
  stop("NSE_RUN_PROFILE must be one of: ",
       paste(ALLOWED_RUN_PROFILES, collapse = ", "))
}
...
OUTPUT_DIR <- file.path(
  DATA_DIR,
  paste0("TGARCH_X_Reviewer_Aligned_", PERIOD_TAG, "_", RUN_PROFILE)
)
...
build_specification_grid <- function() {
  ...
  selected <- switch(
    ...
    "X" = x_grid,
    "ALL" = bind_rows(primary, window_grid, distribution_grid, x_grid)
  )
...
if (RUN_PROFILE == "ALL") {
  warning(
    "RUN_PROFILE=ALL can require substantial runtime. Separate profile runs ",
    "are preferable for auditability and checkpoint recovery."
  )
}
...
cat("Failed GARCH fits remain missing in primary accuracy results.\n")
...
cat("Primary Python file: TGARCH_X_Forecasts_CLEAN.csv\n")
